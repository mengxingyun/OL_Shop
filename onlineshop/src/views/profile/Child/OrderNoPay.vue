...
<el-card class="box-bottom" style="width: 1200px; margin: 0 auto">
    <div class="card-header"> 
        ...
        <el-button type="success" round style="font-size:20px" @click="PayOrder">立即支付</el-button>
        <el-button type="danger" round style="font-size:20px" @click="CancelOrder">取消支付</el-button>
    </div>
</el-card>
</template>
...
import {getOrderDetail, payOrder, cancelOrder } from 'network/order';
...
export default {
    ...
    setup() {
        ...
        const PayOrder = () => {
            payOrder(route.query.order_id).then((res) => {
                console.log(res);
                if(res.flag == true){
        ...
        const CancelOrder = () => {
            cancelOrder(route.query.order_id).then((res) => {
                console.log(res);
                if(res.flag == true){
                    ElMessage.success({
                        message: '成功取消',
